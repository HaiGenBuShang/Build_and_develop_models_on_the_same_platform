...
show_gene_numbers <- function(tumor_expr_dat,training_and_testing_res,
                              predicting_type=c("AIMS","PAM","ClaNC"),class_label){
  switch(predicting_type,
         AIMS={
           c(Gene_pairs_gene_number=str_split(train_and_test_res$training_res$final_model$all.pairs,pattern = "<") %>% unlist() %>% 
               unique() %>% length())
         },
         PAM={
           #Select PAM model with highest MCC
           ...
           pam_info<-capture.output(pamr_survived_genes_2<-pamr.listgenes(fit = PAM_model$training_model,
                                                                          data = PAM_model$traning_and_testing$train_set,
                                                                          threshold = PAM_model$delt,
                                                                          genenames = TRUE)[,"name"])
           
           c(PAM_gene_number=pamr_survived_genes %>% length(),PAM_Spearman_gene_number=pamr_survived_genes_2 %>% length())
         },
         ClaNC={
           if(missing(class_label))
             stop("Please providing class label when using ClaNC!")
           ...
           pam_info<-capture.output(pamr_survived_genes_2<-pamr.listgenes(fit = PAM_model$training_model,
                                                                          ...
                                                                          threshold = PAM_model$delt,
                                                                          genenames = TRUE)[,"name"])
           c(ClaNC_gene_number= ClaNC_model$ClaNC_res$gene_fetures %>% length(),
             PAM_gene_number=pamr_survived_genes %>% length(),PAM_Spearman_gene_number=pamr_survived_genes_2 %>% length())
...
#*Table 3****
#************
#Kappa value of "Gene-pairs" "PAM" "PAM+SPearman"
#"ClaNC" "ClaNC+PAM" + "ClaNC+PAM+Spearman" respectively
...
},y=as.character(1:4),simplify = TRUE) %>% as.data.frame())
...
#MCC value of "Gene-pairs" "PAM" "PAM+SPearman"
#"ClaNC" "ClaNC+PAM" + "ClaNC+PAM+Spearman" respectively
...
#*Table 3****
#************
#gene numbers of "Gene-pairs" "PAM" "PAM+SPearman"
#"ClaNC" "ClaNC+PAM" + "ClaNC+PAM+Spearman" respectively

show_gene_numbers(tumor_expr_dat = SEQC_expr_BGI,
                  training_and_testing_res = train_and_test_res,
                  predicting_type = "AIMS")
show_gene_numbers(tumor_expr_dat = SEQC_expr_BGI,training_and_testing_res = PAM_and_PAM_plus_Cor,
                  predicting_type = "PAM")
show_gene_numbers(tumor_expr_dat = SEQC_expr_CNL,
                  training_and_testing_res = ClaNC_and_PAM_train,
                  predicting_type = "ClaNC",
                  class_label = levels(as.factor(datset_for_model$y)))
...
#**************************************
#*Predict using Pearson Coefficient****
#**************************************
...
#*****************************************************************************************
#*In ClaNC+PAM+Spearman, Kappa changed to 1.00 when Spearman were replaced by Pearson ****
#*****************************************************************************************
