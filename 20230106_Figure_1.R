#****************************************************************************
#*all figures in manuscript by Fig. 1
#****************************************************************************
...
pdf("../Results/Figure/Figure_1b_median_centered.pdf",width = 10,height = 5)
clustering_dat_for_plot %>% arrange(RNA_seq_PAM50,Array_PAM50,RNA_seq_AIMS,Array_AIMS) %>% 
  mutate(barcode=factor(barcode,levels = unique(barcode))) %>% 
  pivot_longer(cols = -barcode,names_to = "Clustering",values_to = "value") %>% 
  mutate(Clustering=factor(Clustering,levels = c("RNA_seq_PAM50","Array_PAM50","RNA_seq_AIMS","Array_AIMS")[4:1])) %>% 
...
pdf("../Results/Figure/Figure_1b_mean_centered.pdf",width = 10,height = 5)
clustering_dat_for_plot %>% arrange(RNA_seq_PAM50,Array_PAM50,RNA_seq_AIMS,Array_AIMS) %>% 
  mutate(barcode=factor(barcode,levels = unique(barcode))) %>% 
  pivot_longer(cols = -barcode,names_to = "Clustering",values_to = "value") %>% 
  mutate(Clustering=factor(Clustering,levels = c("RNA_seq_PAM50","Array_PAM50","RNA_seq_AIMS","Array_AIMS")[4:1])) %>% 
...
pdf("../Results/Figure/Figure_1x.pdf",width = 10,height = 5)
clustering_dat_for_plot %>% 
  left_join(clustering_dat_for_plot_2,by = "barcode",suffix=c("_scale","_original")) %>% 
  arrange(RNA_seq_PAM50_scale,Array_PAM50_scale,RNA_seq_AIMS_scale,Array_AIMS_scale,
          RNA_seq_PAM50_original,Array_PAM50_original,RNA_seq_AIMS_original,Array_AIMS_original) %>% 
  mutate(barcode=factor(barcode,levels = unique(barcode))) %>% 
  pivot_longer(cols = -barcode,names_to = "Clustering",values_to = "value") %>% 
  mutate(Clustering=factor(Clustering,
                           levels = c("RNA_seq_PAM50_scale","Array_PAM50_scale",
                                      "RNA_seq_AIMS_scale","Array_AIMS_scale",
                                      "RNA_seq_PAM50_original","Array_PAM50_original",
                                      "RNA_seq_AIMS_original","Array_AIMS_original")[8:1])) %>% 
  ggplot() +
  geom_tile(aes(x=barcode,y=Clustering,fill=factor(value))) +
  #scale_fill_viridis(discrete=FALSE) +
  scale_fill_brewer(palette = "Set1") +
...
ann_colors <- list(data_type= c(`RNA-seq` ="#A04EF6", Array = "#E7D044"),
                   samples=setNames(rainbow(ncol(RNA_seq_expr_PAM50)),
                                    intersect(colnames(RNA_seq_expr_for_clust),colnames(array_expr_for_clust))))
pdf("../Results/Figure/Figure_1d_supp.pdf",width = 15,height = 15)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = colnames(combined_dat),
         annotation_col = annotation_col[,-2,drop=FALSE],fontsize_col = 1,clustering_method = "ward.D",
...
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         ...
         annotation_col = annotation_col,fontsize_col = 2,clustering_method = "ward.D",annotation_colors = ann_colors,
         color = colorRampPalette(c("green", "black", "firebrick3"))(101),breaks = seq(-2,2,length.out=101))
dev.off()


pdf("../Results/Figure/Figure_1d_median_centering.pdf",width = 15,height = 15)
...
annotation_col <- data.frame(data_type=rep(c("RNA-seq","Array"),
                                           times=c(ncol(RNA_seq_expr_PAM50),ncol(array_expr_PAM50))),
                             samples=intersect(colnames(RNA_seq_expr_for_clust),colnames(array_expr_for_clust)))
rownames(annotation_col) <- colnames(combined_dat1)
pheatmap(mat = combined_dat1,labels_row = rep('',nrow(combined_dat1)),
         labels_col = colnames(combined_dat1),
         annotation_col = annotation_col[,-2,drop=FALSE],fontsize_col = 1,clustering_method = "ward.D",
         annotation_colors = ann_colors,
         color = colorRampPalette(c("green", "black", "firebrick3"))(101),breaks = seq(-2,2,length.out=101))
dev.off()
...
pdf("../Results/Figure/Figure_1d_mean_centering.pdf",width = 15,height = 15)
...
pdf("../Results/Figure/Figure_1c_new.pdf",width = 10,height = 5)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = str_replace_all(colnames(combined_dat),".*",""),
...
ann_colors <- list(data_type= c(`RNA-seq` ="#A04EF6", Array = "#E7D044"),
                   samples=setNames(rainbow(ncol(RNA_seq_expr_PAM50)),
                                    intersect(colnames(RNA_seq_expr_for_clust),colnames(array_expr_for_clust))))
pdf("../Results/Figure/Figure_1c_supp.pdf",width = 15,height = 15)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = colnames(combined_dat),
...
rownames(annotation_col) <- colnames(combined_dat)
# ann_colors <- list(data_type= c(`RNA-seq` ="#A04EF6", Array = "#E7D044"))
ann_colors <- list(data_type= c(`RNA-seq` ="#377EB8", Array = "#E41A1C"))

# pdf("../Results/Figure/Figure_1c.pdf",width = 10,height = 5)
pdf("../Results/Figure/Figure_1c.pdf",width = 10,height = 10)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = str_replace_all(colnames(combined_dat),".*",""),
         annotation_col = annotation_col,fontsize_col = 2,clustering_method = "ward.D",annotation_colors = ann_colors,
         color = colorRampPalette(c("green", "black", "firebrick3"))(101),breaks = seq(-2,2,length.out=101))
...
# annotation_col <- data.frame(data_type=rep(c("RNA-seq","Array"),
#                                            times=c(ncol(RNA_seq_expr_PAM50),ncol(array_expr_PAM50))),
#                              samples=(clustering_dat_for_plot %>% filter(RNA_seq_PAM50=="Basal")) %>% rownames())
# rownames(annotation_col) <- colnames(combined_dat)
# 
# ann_colors <- list(data_type= c(`RNA-seq` ="#A04EF6", Array = "#E7D044"),
#                    samples=setNames(rainbow(ncol(RNA_seq_expr_PAM50)),
...
pdf("../Results/Figure/Figure_1f.pdf",width = 10,height = 5)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = str_replace_all(colnames(combined_dat),".*",""),
         annotation_col = annotation_col,fontsize_col = 2,clustering_method = "ward.D",annotation_colors = ann_colors,
         color = colorRampPalette(c("green", "black", "firebrick3"))(101),breaks = seq(-2,2,length.out=101))
...
pdf("../Results/Figure/Figure_1d_new.pdf",width = 10,height = 5)
pheatmap(mat = combined_dat,labels_row = rep('',nrow(combined_dat)),
         labels_col = str_replace_all(colnames(combined_dat),".*",""),
         annotation_col = annotation_col,fontsize_col = 2,clustering_method = "ward.D",annotation_colors = anno_color,
         color = colorRampPalette(c("green", "black", "firebrick3"))(101),breaks = seq(-2,2,length.out=101))
